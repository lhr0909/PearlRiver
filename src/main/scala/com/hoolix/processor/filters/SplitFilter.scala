  ...
  override def handle(event: Event): Event = {
    ...
    if (targetField != null && targetField != "") {
      deepGet(payload, targetField) match {
        case Some(targetValue: String) => if (targetValue != "") {
          ...
          pairs.foreach({
            case (name, value) => deepPut(payload, name, value)
          })
        }
        case None =>
      }
...
//    val (start, end, value) = ctx.get_preview(cfg.target, (-1,-1,""))
    if (value != "") {
      val buffer: Iterable[(Int,Int,String)] = split_with_position(value, delimiter, limit)
